...
/**
 * @ClassName ConsistentHashStrategy
 * @Description 一致性哈希算法
 * 1.传统hash算法  hash(key) % / 机器数 = 余数 （服务器序号）
 * hash负载优点：相同的key可以保证相同的负载（这个特性广泛应用于缓存负载均衡领域）
 * 缺点：但是节点扩展后无法保证， 而且会全局影响其他之前的负载结果。但是一致性hash只会影响到小部分
 * <p>
 * 2.原理: 事先将服务提供者列表hash映射到哈希环上(节点)，当一个请求到来，对请求的key进行hash映射到环上，
 * 然后顺时针寻找，找到的第一个节点就作为负载后的服务提供者
 * <p>
...
public class ConsistentHashStrategy {
    ...
    public String getServer(String key) {
        ...
        log.info("{}:key的hash值：{}", key, hash);
        // 顺时针寻找找到的第一个节点
        SortedMap<Integer, String> sortedMap = hashRing.tailMap(hash);

        String targetNode;
        if (sortedMap.isEmpty()) {
            // 如果sortedMap为空，表示hash寻找到了哈希环的尾部，直接寻找到hash环第一个节点即可
            targetNode = hashRing.get(hashRing.firstKey());
        } else {
            targetNode = sortedMap.get(sortedMap.firstKey());
        }
        // 最后把找到的虚拟节点名称截取一下就可以获得真实节点的名称
        return StringUtils.isEmpty(targetNode) ? null : targetNode.substring(0, targetNode.indexOf("&&"));
    }
